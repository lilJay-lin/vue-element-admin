  ...
  <div :class="[containerClass]">
    <div class="filter-container">
      ...
      <el-select  v-if="isMain"  @change='handleFilter' style="width: 120px" class="filter-item" v-model="listQuery.locked" placeholder="状态">
        <el-option :key="'all'" :label="'是否有效'" :value="''"></el-option>
    ...
    <el-table :key='tableKey' @selection-change="handleSelectionChange" :data="shopAccount.records" v-loading="listLoading" border fit highlight-current-row style="width: 100%">
      <el-table-column v-if="isMain && checkPermission(permissionConstant.shop_d)"
        type="selection"
        width="55">
      </el-table-column>
      ...
      <template  v-if="isMain" >
        <el-table-column align="center" label="操作" width="150" >
          <template scope="scope">
            ...
            <el-button v-if="checkPermission(permissionConstant.shop_d)"  size="small" type="danger" @click="handleModifyStatus(scope.row, true)">删除</el-button>
          </template>
        </el-table-column>
      </template>
      <template v-else>
