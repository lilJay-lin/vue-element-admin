  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        this.$store.dispatch('GetAllRegisters', this.listQuery).then(() => {
      ...
      delete (ids, msg) {
        this.$confirm(msg).then(() => {
          this.$store.dispatch('DelRegisters', { ids }).then(() => {
            this.$message({
              message: '操作成功',
              type: 'success'
            })
      ...
      handleDelRelation (register) {
        const me = this
        me.$store.dispatch('DelRegisters').then(() => {
          me.temp.registers.splice(me.temp.registers.indexOf(register), 1)
        })
      },
      handleUpdate(row) {
