  ...
  <el-dialog  :title="textMap[dialogStatus]" :visible="dialogFormVisible" :before-close="cancel" size="small">
    <el-form class="small-space" :model="detail" :rules="detailRules" ref="detailForm" label-position="left" label-width="100px" style='width: 320px;margin-left:50px'>
      ...
      <el-form-item label="缩略图">
        <template v-if="checkPermission(permissionConstant.advertisement_u) && dialogStatus !== 'info'">
          <upload
            :action="image.action"
            @change="image.change"
    ...
    <div slot="footer" class="dialog-footer">
      ...
      <template v-else>
        <el-button type="primary" v-if="checkPermission(permissionConstant.advertisement_u)" @click="update">确 定</el-button>
      </template>
    </div>
  </el-dialog>
</template>
