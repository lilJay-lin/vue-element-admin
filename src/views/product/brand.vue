<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-popover
        ...
        <el-form class="small-space" :model="temp" :rules="tempRules" ref="detailForm" label-position="left" label-width="50px" style='width: 200px'>
          <el-form-item label="名称"  prop="name">
            <el-input v-model="temp.name" @keyup.enter.native="submit"></el-input>
          </el-form-item>
        </el-form>
      ...
      <el-button v-popover:add @click="showPopover" class="filter-item" style="margin-left: 10px" type="primary" icon="edit">添加</el-button>
    </div>
    <el-table :key='tableKey' :data="editBrands.records" v-loading.body="listLoading" border fit highlight-current-row >
    ...
    <div v-show="!listLoading" class="pagination-container">
      <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page.sync="listQuery.currentPage"
                     :page-sizes="[10,20,30, 50]" :page-size="listQuery.pageSize" layout="total, sizes, prev, pager, next, jumper" :total="editBrands.pageInfo.totalRow">
      </el-pagination>
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapGetters(['brands']),
      editBrands () {
        const brands = Object.assign({}, this.brands)
        brands.records = brands.records.map((brand) => {
          this.$set(brand, 'edit', false)
          return brand
        })
        return brands
      }
    ...
    methods: {
      getList() {
        ...
        this.$store.dispatch('GetAllBrands', this.listQuery).then(() => {
      ...
      handleModifyStatus(row, status) {
        ...
        if (status === '0') {
          promise = this.$confirm('确认删除品牌：' + row.name + '？')
        }
        promise.then(() => {
          this.$store.dispatch('DelBrands', { ids: [row._id], data: { status } }).then(() => {
            this.$message({
              message: '操作成功',
              type: 'success'
            })
      ...
      handleDelRelation (brand) {
        const me = this
        me.$store.dispatch('DelBrands').then(() => {
          me.temp.editBrands.splice(me.temp.editBrands.indexOf(brand), 1)
        })
      },
      handleUpdate(row) {
        ...
        delete detail.edit
        this.$store.dispatch('UpdateBrandDetail', detail).then(() => {
          this.$message({
            message: '操作成功',
            type: 'success'
          })
      ...
      submit () {
        this.$refs.detailForm.validate((valid) => {
          if (valid) {
            this.$store.dispatch('CreateBrand', this.temp).then(() => {
              this.getList()
              this.cancel()
            }, () => {})
          }
